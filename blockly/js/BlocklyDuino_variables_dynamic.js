...
Blockly.VariablesDynamic.flyoutCategory = function(workspace) {
  ...
  xmlList.push(button);
  button = document.createElement('button');
  button.setAttribute('text', Blockly.Msg['NEW_PANDA_VARIABLE']);
  button.setAttribute('callbackKey', 'CREATE_VARIABLE_PANDA');
  xmlList.push(button);
  ...
  workspace.registerButtonCallback('CREATE_VARIABLE_PANDA',
      Blockly.VariablesDynamic.onCreateVariableButtonClick_Colour);
...
Blockly.VariablesDynamic.flyoutCategoryBlocks = function(workspace) {
  ...
  if (variableModelList.length > 0) {
    if (Blockly.Blocks['variables_set_dynamic']) {
      ...
      var gap = 24;
      var blockText = '<xml>' +
          '<block type="variables_set_panda" gap="' + gap + '">' +
    ...
    if (Blockly.Blocks['variables_get_panda']) {
      variableModelList.sort(Blockly.VariableModel.compareByName);
      for (var i = 0, variable; variable = variableModelList[i]; i++) {
        var blockText = '<xml>' +
            '<block type="variables_get_panda" gap="8">' +
